...
if [ "$1" = 'mysqld' ]; then
	# Get config
	#DATADIR="$("$@" 2>/dev/null | awk '$1 == "datadir" { print $2; exit }')"
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		echo 'Finished mysql_install_db' $SOCKET

		mysqld --user=mysql --datadir="$DATADIR" --skip-networking --basedir=/usr --pid-file="$PIDFILE" &
